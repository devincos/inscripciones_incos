...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $mensaje = 'Datos de materia registrados satisfactoriamente!!!oooo';
    $error = 'Error al registrar, revise los datos que esta ingresandooooo';

    if (empty($datos['id_materia'])) {
        if ($materia_nueva->insertarMateria($datos['nom_materia'], $datos['nro_hrs'], $datos['anio_materia'],$datos['id_carrera'])) {
            ...
            header("Location:" . Config::baseurl() . "views/materia/form.php?error=$error&nom_materia=" . $datos['nom_materia'] . "&nro_hrs=" . $datos['nro_hrs'] . "&anio_materia=" . $datos['anio_materia']. "&id_carrera=" . $datos['id_carrera']);
            exit();
        }
    }else{
